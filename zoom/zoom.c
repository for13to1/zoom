...
static char rcsid[] = "$Header: /.dub/dub/Repository/src/zoom/zoom/zoom.c,v 1.2 1998/12/13 23:07:32 pahvant Exp $";
...
int zoom_quiet = 0;		/* set to 1 to keep still */
...
double s, u;	/* scale and offset */
{
	...
	&& INTEGER((u/s-.5)/a->scale))) {
	    if (!str_eq(filter->name, "point") && ! zoom_quiet)
...
Filt *xfilt, *yfilt;
{
    ...
    ax.wid = (int) ceil(2.*ax.supp);
    ay.wid = (int) ceil(2.*ay.supp);
    ...
    bc.x0 = (int) ceil(m->sx*(a.x0-ax.supp)+m->tx+EPSILON);
    bc.y0 = (int) ceil(m->sy*(a.y0-ay.supp)+m->ty+EPSILON);
    bc.x1 = (int) floor(m->sx*(a.x1+ax.supp)+m->tx-EPSILON);
    bc.y1 = (int) floor(m->sy*(a.y1+ay.supp)+m->ty-EPSILON);
...
int square, intscale;	/* square mapping? integer scales? */
{
	...
	if (map.sx>map.sy) {	/* use the smaller scale for both sx and sy */
	    map.sx = map.sy;
	    b->nx = (int) ceil(a->nx*map.sx);
	}
	else {
	    map.sy = map.sx;
	    b->ny = (int) ceil(a->ny*map.sy);
	}
    }
    if (intscale) {
	if (map.sx>1.) {
...
Weighttab *wtab;
{
    ...
    i0 = (int) (cen-ap->supp+.5);
    i1 = (int) (cen+ap->supp+.5);
    if (i0<0) i0 = 0;
    if (i1>len) i1 = len;
    if (i0>=i1) {
...
short *map;		/* mapping table */
{
	...
	if (scale==1.)			/* no scale change, translation only */
		 ...
		 */
		split = (int) floor((tran+scale*asupp-.5)/(1.-scale)-b0+1.);
	    }
	    else {			/* minify */
		/*
		 ...
		 */
		split = (int) ceil((tran+scale*asupp)/(1.-scale)-b0);
		/*
