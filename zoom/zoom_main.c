...
extern void zoom_opt(Pic *apic, Window_box *a, Pic *bpic, Window_box *b,
                     Filt *xfilt, Filt *yfilt, int square, int intscale);
extern void zoom_continuous(Pic *apic, Window_box *awin,
                            Pic *bpic, Window_box *bwin,
...
char **av;
{
	...
	if (av[i][0]=='-')
		...
		a.x0 = (int) atof_check(av[++i]);
		a.y0 = (int) atof_check(av[++i]);
		a.nx = (int) atof_check(av[++i]);
		a.ny = (int) atof_check(av[++i]);
		aboxset = 1;
	    }
	    else if (str_eq(av[i], "-d") && ok(i+4<ac, "-d")) {
		b.x0 = (int) atof_check(av[++i]);
		b.y0 = (int) atof_check(av[++i]);
		b.nx = (int) atof_check(av[++i]);
		b.ny = (int) atof_check(av[++i]);
		bboxset = 1;
	    }
	    else if (str_eq(av[i], "-sw") && ok(i+4<ac, "-sw")) {
		a.x0 = (int) atof_check(av[++i]);
		a.y0 = (int) atof_check(av[++i]);
		a.x1 = (int) atof_check(av[++i]);
		a.y1 = (int) atof_check(av[++i]);
		aboxset = 1;
	    }
	    else if (str_eq(av[i], "-dw") && ok(i+4<ac, "-dw")) {
		b.x0 = (int) atof_check(av[++i]);
		b.y0 = (int) atof_check(av[++i]);
		b.x1 = (int) atof_check(av[++i]);
		b.y1 = (int) atof_check(av[++i]);
		bboxset = 1;
	    }
		...
		zoom_debug = (int) atof_check(av[++i]);
            else if (str_eq(av[i], "-quiet") || str_eq(av[i], "-q"))
		zoom_quiet = 1;
    ...
    if (dstfile[0] == '-' || !str_eq(srcfile,dstfile)) 
      {
	apic = pic_open(srcfile, "rb");
	if (!apic) {
	  fprintf(stderr, "can't get %s\n", srcfile);
	  exit(1);
	}
      }
    bpic = pic_open(dstfile, "wb");
    if (!bpic) {
	fprintf(stderr, "can't get %s\n", dstfile);
	exit(1);
    }
	...
	*/
       if (! aboxset)        pic_get_window(apic, (Window_box *)&a);
	...
	*/
       ...
       if (!zoom_quiet)
	{
	  ...
	  if (yfilt->printproc) 
       ...
       if (! mset) zoom_opt(apic, &a, bpic, &b, xfilt, yfilt, square, intscale);
       else   zoom_continuous(apic, &a, bpic, &b, &m, xfilt, yfilt);

     } while (pic_next_pic(apic) && pic_next_pic(bpic));
