...
static char rcsid[] = "$Header: /.dub/dub/Repository/src/zoom/zoom/scanline.c,v 1.1.1.1 1998/12/11 01:15:34 pahvant Exp $";
#include <stdio.h>
...
Scanline_rgb2 *bbuf;
{
    ...
    register short *wp;
    register int sumr, sumg, sumb, suma, af;
    int b;

    for (b=0; b<bn; b++, wtab++, bbuf++) {
	/* start sum at 1<<shift-1 for rounding */
	sumr = sumg = sumb = suma = 1 << (shift-1);
...
register Scanline_rgb4 *accum;
{
	...
	accum->a += (short)weight * (short)UCHAR(buf->a);
    }
}
...
register Scanline_rgb1 *bbuf;
{
	...
	bbuf->a = PEG((accum->a+half) >> shift, t);
    }
}
